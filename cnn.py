...
class charnn(object):
	...
	def __init__(self, rng, d, s, k, cl, W=None, C=None, b=None):
		self.d = d
		self.s = s
		self.k = k
		self.cl = cl
...
class sentnn(object):
	...
	def __init__(self, rng, cd, cs, ck, cl, wd, ws, wk, wl, cW=None, wW=None, cC=None, wC=None, cb=None, wb=None):
...
def train(char_vocab_size, word_vocab_size, sentvec, inp, labels, learning_rate=0.1, training_epochs=10):
	...
	train_cnn = theano.function([idx], [T.mean(cost)], updates=updates, givens={x:inp[idx], s:sentvec[idx], l:len(sentvec[idx]), y:labels[idx]}, mode="FAST_RUN")
